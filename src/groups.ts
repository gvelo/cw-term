...
export class Groups extends Array<string> {
    ...
    constructor(lessonLevel: number, mainCharacter: string, groupCount: number, maxSecondaryCharacters: number) {

        super();

        let availableSecondaryCharacters = MORSE_SYMBOLS.slice(0, lessonLevel + 1).filter(char => char !== mainCharacter);
        ...
        let secondaryCharsToAddCount = (availableSecondaryCharacters.length > maxSecondaryCharacters)
            ? maxSecondaryCharacters
            : availableSecondaryCharacters.length;

        let secondaryRepetitions = Math.floor(secondaryCharCount / secondaryCharsToAddCount);
        ...
        if (availableSecondaryCharacters.length > secondaryCharsToAddCount) {
            availableSecondaryCharacters = shuffleArray(availableSecondaryCharacters).slice(0, secondaryCharsToAddCount)
        }

        const secondayDistribution = buildSecDistribution(availableSecondaryCharacters, secondaryRepetitions, secondaryRemainder);
...
function buildSecDistribution(secondayChars: Array<string>, repetitions: number, remainder: number): Array<{ char: string, repetitions: number }> {
...
export function buildGroups(lesson: number, mainChar: string, numOfGroups: number, maxSecCharacters: number): Groups {
